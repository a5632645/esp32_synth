...
typedef struct {
    ...
    void(*AeraPainted)(void* context, int x, int y, int w, int h) = nullptr;
 ...
 * @todo  it should only contain flush method, the draw methods should abstract to @graphic class
 * 
 * you have to implement these method for your screen driver
 * all the coordinate is limit in the bound you provide
 */
class LLContext {
public:
    ...
    virtual void AeraPainted(Bound paint_aera) {/* if use fsmc or ltdc, do not need flush */}
